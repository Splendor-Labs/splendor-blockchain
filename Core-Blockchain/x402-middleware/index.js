...
class SplendorX402Middleware {
  constructor(options = {}) {
    ...
    // Asset to use for payments (ERC-20 token address). Defaults to native (0x0) if not provided.
    this.asset = options.defaultAsset || '0x0000000000000000000000000000000000000000';
    // Decimals for ERC-20 assets (used only if asset != zero address)
    this.assetDecimals = typeof options.assetDecimals === 'number' ? options.assetDecimals : 18;
  ...
  dollarToWei(dollarAmount) {
    ...
    // If using native SPLD (zero address), keep legacy USD->SPLD conversion at 18 decimals
    if (this.asset === '0x0000000000000000000000000000000000000000') {
      // Assuming 1 SPLD = $0.38, so $0.001 = 0.00263 SPLD
      const spldAmount = amount / 0.38;
      const wei = Math.floor(spldAmount * 1e18);
      return '0x' + wei.toString(16);
    }
    // For ERC-20 tokens, interpret pricing as token units and scale by token decimals
    const scale = 10 ** this.assetDecimals;
    const units = Math.floor(amount * scale);
